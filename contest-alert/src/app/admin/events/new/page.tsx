...
export default function CreateEventPage() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      list.push(newEvent);
      localStorage.setItem("rit_events", JSON.stringify(list));
    ...
    <div className="min-h-[100dvh] bg-[var(--background)]">
      ...
      <main className="lg:ml-[var(--sidebar-width)] min-h-[100dvh] pb-16">
        ...
        <header className="sticky top-0 z-20 glass border-b border-[var(--surface-border)] px-6 lg:px-8 py-4 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-[var(--foreground-muted)] uppercase tracking-wider">New Event Creation</span>
        </header>
          ...
          <motion.div variants={fadeUp} initial="hidden" animate="visible" className="card-bezel">
            <form onSubmit={handleSubmit} className="card-bezel-inner p-6 sm:p-8 space-y-6">
              ...
              <div className="flex items-center gap-2 border-b border-[var(--surface-border)] pb-3">
                ...
                <div>
                  <h2 className="text-base font-bold">Launch New Campus Event</h2>
                  <p className="text-[10px] text-[var(--foreground-muted)]">Fills all event metadata to publish across RIT campuses</p>
                </div>
              </div>
              ...
              <div className="space-y-1">
                <label className="text-xs font-semibold text-[var(--foreground-secondary)]">Cover Image URL (Optional)</label>
                <input 
                  type="url" placeholder="https://picsum.photos/... or leave blank for random"
